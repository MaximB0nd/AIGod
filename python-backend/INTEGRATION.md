...
- **default_agents** — шаблоны (Копатыч, Гермиона, Нарратор, Суммаризатор и т.д.). Заполняется при первом старте из `app/data/default_agents_data.py`. В режиме `circular` Рассказчик и Суммаризатор добавляются автоматически.
...
- `circular` — агенты общаются по кругу. В каждом чате участвуют Рассказчик (🎭) и Суммаризатор (📊). До 50 раундов.
