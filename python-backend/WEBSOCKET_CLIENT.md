...
| sender        | string | Имя отправителя: `"user"`, имя агента, `"🎭 Рассказчик Нарратор"`, `"📊 Сводка Суммаризатор"`, `"Система"` |
| agentId       | string?| ID агента (если есть). `null` — пользователь, Рассказчик, Суммаризатор, Система |
| timestamp     | string | ISO 8601                                  |
| agentResponse | string?| Ответ агента (если уже сгенерирован). В оркестрации ответы приходят отдельными сообщениями |

В режиме `circular`: `sender` может быть именем агента, `"🎭 Рассказчик Нарратор"` (нарратив), `"📊 Сводка Суммаризатор"` (сводка раунда), `"Система"` (финальный ответ или «Раунд X завершён»).
