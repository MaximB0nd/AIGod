...
**См. также:**
- `API_DOCS.md` — полный REST-справочник
- `CONNECTION.md` — быстрый старт подключения

---

## Подключение

| Режим        | WebSocket URL               | REST API        |
|--------------|-----------------------------|------------------|
| Локально     | `ws://localhost:8000`        | `http://localhost:8000` |
...
1. **Подключаться при открытии чата** — WebSocket должен быть подключён до того, как пользователь отправит сообщение. Иначе broadcast не дойдёт до клиента.
2. **Загружать историю** — при открытии чата вызови `GET /api/rooms/{roomId}/messages` или `GET /api/rooms/{roomId}/feed`, чтобы показать существующие сообщения.
3. **Смена комнаты** — закрой текущие WS и подключись к новым URL с новым `roomId`.
4. **Смена токена** — после refresh токена переподключай WebSocket с новым `token`.
5. **Reconnect** — при `onclose` можно делать `setTimeout` и повторное подключение (без спама).
6. **Ping** — отправляй `{"type": "ping"}` раз в 20–30 сек, чтобы соединение не рвалось.
7. **HTTPS** — в production используй `wss://` на том же хосте, что и `https://`.

---

## Устранение неполадок: чат пустой

Если сообщения сохраняются в БД, но не отображаются в интерфейсе:

1. **WebSocket подключён?** — Подключение должно быть до отправки. Проверь `onopen` и `readyState === WebSocket.OPEN`.
2. **Правильный roomId?** — URL `ws://.../api/rooms/{roomId}/chat` должен совпадать с комнатой, куда отправляешь сообщения.
3. **Логи сервера** — при broadcast с 0 подключений в логе будет: `broadcast room_id=X — 0 подключений, сообщение не доставлено`. Значит клиент не подключился к WS этой комнаты.
4. **Начальная загрузка** — при открытии чата вызови `GET /api/rooms/{roomId}/messages` для показа уже сохранённых сообщений.
