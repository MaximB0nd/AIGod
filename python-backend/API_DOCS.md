...
При удалении автоматически удаляются все сообщения чата, события комнаты, связи между агентами; оркестрация комнаты останавливается.
...
Если `type` равен `"user_message"` или `"chat"`, `description` трактуется как сообщение пользователя в чат — создаётся Message, broadcast, триггер ответов агентов. Используйте для совместимости, если фронтенд отправляет сообщения через events/broadcast.
